...
import { Service, PlatformAccessory, CharacteristicValue, Logger, PlatformConfig, Characteristic } from 'homebridge';
...
export class MolekulePlatformAccessory {
  ...
  async handleActiveGet(): Promise<CharacteristicValue> {
    ...
    if (response.status == 200) this.updateStates(response);
  ...
  async setSpeed(value: CharacteristicValue) {
    ...
    var clamp = Math.round(Math.min(Math.max((value as number)/20, 1), 5));
  ...
  updateStates(response:any) {
    response = response.json();
    for (let i = 0; i<(Object.keys(response["content"]).length); i++) if (response["content"][i]["serialNumber"] == this.accessory.context.device.serialNumber){
